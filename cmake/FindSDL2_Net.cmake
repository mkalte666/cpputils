...
#
#  SDL2::SDL2_Net - Imported target to use for building a library
#  SDL2_NET_FOUND - True if SDL2 was found.
#  SDL2_NET_DYNAMIC - If we found a DLL version of SDL (meaning you might want to copy a DLL from SDL2::SDL2_Net)
#
...
if(APPLE AND "${SDL2_NET_LIBRARY}" MATCHES "(/[^/]+)*.framework(/.*)?$")
	...
	if("${SDL2_NET_FRAMEWORK}" MATCHES "(/[^/]+)*.framework$")
		...
		find_path(SDL2_NET_INCLUDE_DIR
			NAMES
			SDL_NET.h # this file was introduced with SDL2
			HINTS
...
if(WIN32 AND SDL2_NET_LIBRARY)
    ...
    get_filename_component(SDL2_NET_RUNTIME_LIBRARY_FOLDER ${SDL2_NET_RUNTIME_LIBRARY} DIRECTORY)
    file(GLOB SDL2_NET_RUNTIME_LIBRARY_ADDITIONALS ${SDL2_NET_RUNTIME_LIBRARY_FOLDER}/*.*)
endif()
...
if(SDL2_NET_FOUND)
	if(NOT TARGET SDL2::SDL2_Net)
		...
		if(WIN32 AND SDL2_NET_RUNTIME_LIBRARY)
			...
			elseif(_SDL2_NET_framework AND SDL2_NET_LIBRARY MATCHES "(/[^/]+)*.framework$")
		...
		if(APPLE)
			...
			if(SDL2_NET_COCOA_LIBRARY)
				set_target_properties(SDL2::SDL2_Net PROPERTIES
						IMPORTED_LINK_INTERFACE_LIBRARIES ${SDL2_NET_COCOA_LIBRARY})
			endif()
		endif()
		...
		if(MINGW)
			...
			if(SDL2PC_LIBRARIES)
				...
				if(SDL2_NET_MINGW_LIBRARY)
					set(SDL2MAIN_LIBRARIES ${SDL2_NET_MINGW_LIBRARY} ${SDL2MAIN_LIBRARIES})
				endif()
				if(SDL2_NET_MWINDOWS_LIBRARY)
					set(SDL2MAIN_LIBRARIES ${SDL2_NET_MWINDOWS_LIBRARY} ${SDL2MAIN_LIBRARIES})
				endif()
			endif()
		endif()
	endif()
